    ...
    ui(new Ui::Widget)
{
    ...
    query.exec("insert into student(id, name, age, score) values(1, '二狗', 10, 35)");

    //绑定值的方式，文档中给出了四种方式
    //方式一
    query_1.prepare("insert into student(name, age, score) values(:name, :age, :score)");
    ...
    //方式二
    query_2.prepare("insert into student(name, age, score) values(:name, :age, :score)");
    ...
    query_6.prepare("insert into student(name, age, score) values(:name, :age, :score)");
    QVariantList nameList_2;
    nameList_2 << "王八蛋" << "龟孙" << "都是";
...
void Widget::on_pushButton_Delete_clicked()
{
     ...
     * If open is true (the default) and the database connection is not already open
     * it is opened now.
     *
     * 返回数据库连接，该数据库链接必须已经使用addDatabase()添加过了的。如果连接没开启参数open
     * 为true，则开启该数据库链接.
    */
    QMessageBox::StandardButton button = QMessageBox::question(this, tr("Hint"),
                                                               tr("Do is right now?"));
...
void Widget::on_pushButton_Select_clicked()
{
    ...
    /*
     * setFiletr()方法更简单，只需要输入where语句即可。
     * 上诉函数在调用完select()函数后生效
    */
    QString sql = QString("select * from student where name = '%1'").arg(ui->lineEdit_2->text());
    QSqlQuery query;
    query.exec(sql);
    while(query.next()){
        qDebug() << query.value(1).toString().toUtf8().data()
